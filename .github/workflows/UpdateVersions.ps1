...
$changeLog = ConvertFrom-Json -InputObject "[]";
...
$repoPath = "$($basePluginFolderPath)Repo.json";
$basePluginDefinitionJsonPath = "$($basePluginFolderPath)\$($pluginAssesmblyName).json"
if ((Test-Path $repoPath) -eq $false){
    Copy-Item -Path $basePluginDefinitionJsonPath -Destination $repoPath
}

$repoObject = Get-Content -Path $repoPath | ConvertFrom-Json
...
Set-Content -Path $repoPath -Value (ConvertTo-Json -InputObject $repoObject)
